modded class AnimalBase
{
	override void EEHitBy(TotalDamageResult damageResult, int damageType, EntityAI source, int component, string dmgZone, string ammo, vector modelPos, float speedCoef)
	{
		...
		//Print("[BorgAmrs--EEHitBy--AnimalBase] damageResult=<" +damageResult.GetDamage("","")+ ">] ammo=<" +ammo+">]");
		PlayerBase z_SourcePlayer = PlayerBase.Cast(source);
		ammo.ToLower();
		
		if ( z_SourcePlayer && z_SourcePlayer.IsWearingArms() && ammo.Contains("fist"))
		{
			//Print("[BorgAmrs--EEHitBy--AnimalBase] " + z_SourcePlayer.GetIdentity().GetName() + " is wearing CyborgArms");
			...
			//Print("[BorgAmrs--EEHitBy--AnimalBase] Source dir=<" +z_Sdir+">");
			dBodyApplyImpulse( this, z_Sdir * dBodyGetMass(this) * 4.89); //keep?
			//dBodyApplyImpulseAt( this, z_Sdir * dBodyGetMass(this) * 4.89, this.GetPosition() + z_Sdir); //keep?
